...
Output example:
```
Backup archive successfully created /root/skale-node-backup-2023-04-20-09-24-53.tar.gz
```
...
```shell
scp user@NODE_IP:/root/skale-node-backup* /path/to/create/local/backup
scp user@NODE_IP:/root/.env /path/to/create/local/.env
```
...
```shell
scp /path/to/create/local/backup user@NEW_NODE_IP:/root/
scp /path/to/create/local/.env user@NEW_NODE_IP:/root/
```

== Turn off the 18.04 node

[IMPORTANT]
If you have more then one node, please do this and the following step node by node.
