...
* >= {attached-size} TB (1.9 TB actual measure) attached and unformatted non-boot storage (Note that {attached-size} TB storage results in < {attached-size} TB actual physical storage)
* >= {swap-size} GB swap
...
* xref:index.adoc#_mainnet_versions_for_set_up[Mainnet Versions]
